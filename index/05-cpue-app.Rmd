...
interviews and aggregated to fishing locality (defined spatial regions) and
...
standardization covariates across large regulatory changes in 1996 (TODO insert
details).
...
fit to log-transformed data or a Gamma GLM with a log link (e.g. TODO-REFS). The
...
```{r cpue-quantile-residuals, fig.asp=0.8, fig.cap="Histograms of randomized quantile residuals for the CPUE GLMM standardization models. The histograms illustrate the actual density distribution of 10,000 randomly selected randomized quantile residuals. The dashed lines show the probability density for a normal distribution with the same standard deviation."}
