...
*TO DO: Add info on length-stratification in surveys.*
...
1. `SAMPLE_ID` not in 173726, 173740, 191471, 184243, 184159, 215903, or 223726: these samples were coded as Pacific Cod but have size composition inconsistent with the species. These species were therefore excluded from further analysis.
...
vb_fits <-
  map(surveys_list, function(.survey) {
    map(sexes, function(.sex) {
      ...
      fit_vb(.dat, .sex, method = "mcmc", chains = 2L, uniform_priors = TRUE,
        cores = 1L, control = list(adapt_delta = 0.8))
    })
  })
```
...
get_par_df(vb_fits, 1) %>%
  ...
  knitr::kable(
    caption = "von Bertalanffy growth coefficients for Pacific Cod in the Hecate Strait (SYN HS), Queen Charlotte Sound (SYN QCS) and West Coast Vancouver Island (SYN WCVI) synoptic surveys.",
...
inner_join(est, se) %>%
  ...
  dplyr::select(Survey,Term, Estimate, Conf.low, Conf.high) %>%
   knitr::kable(caption = "Coefficients from the logistic regression maturity ogives.",
     booktabs = TRUE, linesep = "", digits=2, escape=FALSE)
```
...
plyr::ldply(mat_fits, function(.x)
  ...
  arrange(Survey, Sex) %>%
  knitr::kable(
    caption = "Estimates of age at 50\\% maturity from the logistic regression maturity ogives.", booktabs = TRUE, linesep = "", digits=1)
```
...
The calculation of annual mean weight was done in the following steps. The same steps were used in both areas 3CD and 5ABCD. The same length-weight relationship was use for all quarters, but differ by area. The values of the length-weight parameters used are:

*3CD*

- `r paste0("$\\alpha = ", .ALPHA3, "$")`
- `r paste0("$\\beta = ", .BETA3, "$")`

*5ABCD*

- `r paste0("$\\alpha = ", .ALPHA, "$")`
- `r paste0("$\\beta = ", .BETA, "$")`
...
*TO DO: complete or delete this as time permits*
...
The number of age records in each length category in the age sample was then tabulated using the R function *table*. Numbers were then converted to proportions. Resulting annual ALKs with 2 cm length bins are shown in Tables XX (Hecate Strait/Queen Charlotte Sound) and XX (West Coast Vancouver Island), where the sum of age proportions in each length bin is 1.
