...
Length, weight and maturity data from the Hecate Strait, Queen Charlotte Sound, and West Coast Vancouver Island synoptic surveys were extracted from the GFBio database using the following criteria:
...
We extracted length, weight, and age data from the trawl surveys from the GFBio database, using the following criteria:

1. `SPECIES_CODE` = 222: Pacific Cod.

1. `TRIP_SUB_TYPE_CODE` = 2 or 3: research trips.

1. `SAMPLE_TYPE_CODE` = 1 or 2 or 7 or 8: random or total samples.
...
Length and weight data from commercial bottom trawl vessels were extracted from the GFBio database, using the following criteria:
...
1. `SAMPLE_TYPE_CODE` = 1 or 2 or 6 or 7
...
```{r age-mat-coefs, fig.pos = "H", fig.cap = "Rearranged coefficients from the logistic regression maturity ogives. Parameters are in logit or log odds space. The intercept refers to the log odds of maturity for a theoretical fish of age zero.", fig.width=6, out.width="4.5in"}
