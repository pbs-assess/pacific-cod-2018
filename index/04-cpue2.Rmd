...
# plot_pcod_dist(cpue_dat, y = "hours_fished", ylab = "Effort (hr)",
...
g2_dist <- plot_pcod_dist(filter(cpue_dat, spp_catch > 0),
  y = "hours_fished",
  ylab = "Effort (hours)",
...
cpue_dat$month <- relevel(as.factor(sprintf("%02s", cpue_dat$month)), ref = "04")
```
...
if (!file.exists(here::here("data/generated/cpue-models-re.rds"))) {
  m_re <- plyr::mlply(torun, function(formula, area, formula_version) {
    ...
    if (area == "3CD") {
      df$depth_bin <- relevel(df$depth_bin, ref = "150")
    } else {
      df$depth_bin <- relevel(df$depth_bin, ref = "125")
    }
    message("Fitting area ", area, " and model ", formula)
    fit_cpue_index(df,
      formula_binomial = as.formula(paste("pos_catch ~", formula)),
      formula_gamma =
        as.formula(paste("spp_catch / hours_fished ~", formula))
    )
  })
...
# predictions_re <- predictions_re %>% filter(area == "3CD")
g_binomial <- plot_re_predictions(predictions_re, "Binomial")
g_gamma <- plot_re_predictions(predictions_re, "Gamma")
cowplot::plot_grid(g_binomial, g_gamma, ncol = 1)
```
...
arith_cpue <- out_trip %>% filter(area %in% areas) %>% 
...
if (!file.exists(here::here("data/generated/1996-cpue-models-re.rds"))) {
  m1996_re <- plyr::mlply(torun[,,drop=FALSE], function(formula, area, formula_version) {
    ...
    if (area == "3CD") {
      ...
      df$depth_bin <- relevel(as.factor(df$depth_bin), ref = "150")
    } else {
      df$latitude <- relevel(as.factor(df$latitude), ref = "52.2")
      df$depth_bin <- relevel(as.factor(df$depth_bin), ref = "125")
    }

    message("Fitting area ", area, " and model ", formula)
    # system.time({
      # m1 <- glmmTMB::glmmTMB(as.formula(paste("(spp_catch / hours_fished) ~", formula)), data = filter(df, pos_catch == 1), family = Gamma(link = "log"), verbose = T)
      # m1 <- lme4::glmer(as.formula(paste("(spp_catch / hours_fished) ~", formula)), data = filter(df, pos_catch == 1), family = Gamma(link = "log"))
    #         m2 <- glmmTMB::glmmTMB(as.formula(paste("pos_catch ~", formula)), data = df, family = binomial(link = "logit"), verbose = T)
...
plot_cpue_index_coefs(m1996_re[[toplot[1]]])
plot_cpue_index_coefs(m1996_re[[toplot[2]]])
```


```{r}
g_1996_binomial <- plot_re_predictions(predictions_1996_re, "Binomial", scale = F)
g_1996_gamma <- plot_re_predictions(predictions_1996_re, "Gamma", scale = F)
cowplot::plot_grid(g_1996_binomial, g_1996_gamma, ncol = 1)
...
plot_re_predictions(predictions_1996_re, "Combined", scale = TRUE) +
  geom_line(data = arith_cpue_1996, aes(year, est), inherit.aes = FALSE, lty = 2)
```
