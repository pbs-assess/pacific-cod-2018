...
fi <- file.path(rootd.data, "historic-cpue-raw-data.rds")
if (!file.exists(fi)) {
...
```{r cpue-historic-bridging-plot, fig.cap="Bridging analysis for CPUE with new localities and new depth threshold.", fig.asp=0.4}
...
```{r bubble-predictors-loc, fig.asp=1.2}
bubble_predictors(cpue_dat, "locality", reorder_group = TRUE, ncol = 1)
```

\clearpage

```{r bubble-predictors-month}
# cpue_dat %>%
#   mutate(month = as.character(month)) %>%
#   bubble_predictors("month")
```

```{r bubble-predictors-depth}
#cpue_dat %>%
...
```{r cpue-plot-depth-distributions-positive, fig.cap="The distribution of average trip depth for trips that caught Pacific Cod. Note the deepening in 3CD and 5AB. (B) The distribution of average trip fishing for trips across all trips. Note the deepening in 3CD and 5AB.", fig.asp=1}
g1 <- plot_pcod_dist(filter(cpue_dat, spp_catch > 0),
  y = "geo_mean_depth",
...
```{r, fig.asp=1}
areas_plot <- unique(attr(m_re, "split_labels")$area)
toplot <- which(attr(m_re, "split_labels")$formula_version == "Full standardization")
g1_coef <- plot_cpue_index_coefs(m_re[[toplot[1]]], type = "tweedie",
  re_name = c("RE locality", "NA")) + ggtitle(areas_plot[1])
g2_coef <- plot_cpue_index_coefs(m_re[[toplot[2]]], type = "tweedie",
  re_name = c("RE locality", "NA")) + ggtitle(areas_plot[2])
cowplot::plot_grid(g2_coef, g1_coef, ncol = 1)
```
...
# zz <- c(m1996_re, .m1996_re[2], .m1996_re[4])
# attr(zz, "split_labels") <- torun[c(1:6,8:13,7,14),]
# predictions_1996_re <- plyr::ldply(zz, predict_cpue_index_tweedie)
...
```{r cpue-modern-coefficients, fig.asp=0.9}
toplot <- which(attr(m1996_re, "split_labels")$formula_version == "Full standardization")
...
```{r cpue-modern-coefficients2, fig.asp=0.8}
plot_cpue_index_coefs(m1996_re[[toplot[2]]], type = "tweedie", 
  re_name = c("RE locality", "RE vessel")) + ggtitle(areas_plot[2])
```

```{r cpue-modern-predictions, fig.asp=0.4, fig.cap="The unconditional combined estimate of standardized CPUE (colours) compared with the unstandardized CPUE time series (black). The first 3 columns illustrate the effect of the individual predictors or random intercepts and the last column combines all 3 elements into a single model."}
