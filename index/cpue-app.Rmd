...
match the resolution of the earlier data. When aggregating this 1991--1995 data,
we removed any trawl events that were longer than five hours, since these are
likely to be data entry errors. During these time periods, the variables depth,
date, and locality are available as model covariates.
...
catchability and relationship between CPUE and the standardization covariates
...
for nonlinear relationships between these predictors and CPUE. For depth, we
used the trawl depth binned into bands that are 25m wide. For latitude, we used
bands that were 0.1 degrees wide. To ensure sufficient data to estimate
a coefficient for each factor level, we limited the range of depth bins to
those that fell within the 0.1% to 99.9% cumulative probability of positive
observations and then removed any factor levels (across all predictors) that
contained fewer than 0.1% of the positive fishing events.
...
relative magnitude should not affect the outcomes of the stock assessment model
because we estimated separate catchabilities for each commercial CPUE index.

## A Tweedie GLMM index standardization model

Fisheries CPUE data contains both zeros and positive continuous values.
A variety of approaches have been used in the fishery literature to deal with
this. One approach has been to fit a delta-GLM (generalized linear model) ---
...
fit to log-transformed data or a Gamma GLM with a log link (e.g. REFS). The
probability of a non-zero CPUE from the first component can then be multiplied
by the expected CPUE from the second component to derive an unconditional
estimate of CPUE. However, this approach suffers from a number of issues:
...
effect intercepts ($\alpha$ symbols) are allowed to vary from the overall
intercept by locality $j$ (\(\alpha^\mathrm{locality}_j\)), locality-year $k$
...
trends through time in both the historic and modern datasets (Figure
...
three years before and after 2010 and in this effect carried through to the
...
with their effects in log space drawn from a normal distribution with
a standard deviation of 0.3 and mean 0; optionally year-locality random effects
drawn from a distribution with a standard deviation of 0.5 and mean of 0; 10
observations per year per locality; a true known CPUE index that, in log space,
followed an auto-regressive process with correlation of 0.3 at lag 1, standard
deviation of 1 and a mean of 2; and Tweedie parameters of $p = 1.6$ and $\phi
= 5$. We generated versions of this dataset with and without the locality-year
