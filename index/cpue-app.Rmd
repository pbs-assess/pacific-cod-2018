...
model, we had to filter and manipulate the available catch and effort data to
generate a dataset appropriate for model fitting. In the following sections we
describe those decisions for the 'historical' (1956--1995) and 'modern' data
(1996--2007). We then describe our index standardization model, explore the
contribution of the various standardization components, and identify the effect
of including or ignoring space-time interactions in our standardization model.

## Defining the 1956--1995 fleet

Groundfish bottom trawl data prior to 1991 was recorded via dockside interviews
and aggregated to fishing locality (defined spatial regions) and trip
combinations. Data from 1991 to 1995 was recorded via logbooks at the
fishing-event (trawl) level. We therefore aggregated the 1991 to 1995 data to
the locality-trip level to match the resolution of the earlier data. When
aggregating this 1991--1995 data we removed any trawl events that were longer
than five hours, since these are likely to be data entry errors. During these
time periods, the variables depth, date, and locality are available as model
covariates.

## Defining the 1996--2007 fleet

Groundfish bottom trawl data from 1996 to present have been recorded to the
fishing-event level in the presence of on-board observers or video monitoring.
We have treated this modern dataset separately from the historic dataset to (1)
take advantage of the higher data resolution that also includes
information on latitude and vessel ID and (2) to avoid assuming a constant
catchability and relationship between CPUE and the standardization predictors
across large regulatory changes in 1996.
...
depth, we used the trawl depth binned into bands that are 25m wide. For
latitude, we used bands that are 0.1 degrees wide. To ensure sufficient data to
estimate a coefficient for each factor level, we limited the range of depth
...
predictors) that contained fewer than 0.1% of the positive fishing events.
...
estimate a difference from. The base level then becomes the predictor value
that is used in the prediction for the standardized index. We chose the most
frequent factor level as the base level, which is a common choice for these
types of models [@maunder2004a]. For example, we set the base month as the most
common month observed in the dataset filtered for only tows where the species
was caught. This choice of base level only affects the intercept or relative
magnitude of our index because of the form of our model (discussed below). This
relative magnitude should effect the outcomes of the stock assessment model
because we estimated separate catch abilities for each commercial CPUE index.
...
(generalized linear model) --- a model that fits the zero vs. non-zero values
with a logistic regression (a binomial GLM and a logit link) and the positive
values with a linear regression fit to log-transformed data or a Gamma GLM with
a log link (e.g. REFS). The probability of a non-zero CPUE from the first
component can then be multiplied by the expected CPUE from the second component
to derive an unconditional estimate of CPUE. However, this approach suffers
...
1. The delta-GLM approach assumes independence among the two components [e.g.
   @thorson2017].
1. The delta-GLM approach has shown to be insufficiently robust to variable
...
can be fit relatively quickly to large data sets and for models with many fixed
...
coefficients, and $\mu_i$ represents the expected CPUE in a tow. The (random
effect) intercepts are allowed to vary from the overall intercept \(\alpha\) by
locality $j$ (\(\alpha^\mathrm{locality}_j\)), locality-year $k$
(\(\alpha^\mathrm{locality-year}_k\)), and vessel $l$
(\(\alpha^\mathrm{vessel}_l\)) (for the modern dataset only) and are
constrained by normal distributions with respective standardized deviations
denoted by $\sigma$ parameters.
...
For comparison, we calculated the unstandardized timeseries using a similar
...
(2) the confidence intervals have the correct coverage, i.e. the 95\% confidence
intervals contain the true known value at the expected frequency.

## Commercial CPUE standardization results

The unstandardized Pacific Cod catch and total fleet effort show variable
trends through time in both the historic and modern data sets
(Figure \@ref(fig:cpue-catch-effort-ts)). The majority of positive fishing events for
Pacific Cod occurred at depths ranging from approximately 50m to 200m (Figure
\@ref(fig:cpue-depth-hists)). We can see a increase in the total number of
...
\@ref(fig:cpue-bubble-plots-hist-5ABCD)).

For the modern data sets, we see some changes to the depth distribution,
latitude ... The distribution of index standardization variables for the modern
dataset (Figure \@ref(fig:cpue-bubble-plots-modern-3CD),
\@ref(fig:cpue-bubble-plots-modern-5ABCD)).

For the 1956--1995 data set, depth and locality had a moderate effect on the
...
either depth or locality reduce to the 3CD CPUE in the 1970s and increased the
CPUE from the mid 1980s to 1995. Accounting for depth also reduced a spike in
CPUE in 3CD in the mid 1960s.

For the 1996--2017 data set, depth and latitude had the largest effect on the
standardized index, and again had a larger effect in 3CD than 5ABCD (Figure
\@ref(fig:cpue-index-ts-modern)). Accounting for depth or latitude somewhat
decreased the CPUE index for two to three years before and after 2010 and in
this effect carried through to the standardization model with all covariates.
...
via coefficient plots (historic: Figures \@ref(fig:cpue-coef-plot1),
...
such interactions are or are not present, we performed a simulation test. We
parameterized our simulation to approximately match the parameters estimated
from observed data. Our simulation included 20 years of data; 12 localities
drawn from a random effect distribution with a standard deviation of 0.3 and
mean 0; optionally year-locality random effects drawn from a distribution with
...
generated versions of this data set with and without the locality-year
interactions and then fit standardization models to those data sets that either
allowed for or ignored locality-year interactions.

By including locality-year random effects we allowed for each locality to have
