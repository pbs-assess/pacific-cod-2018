...
contribution of the various standardization components, and explore the
decision to include space-time interactions in our standardization model.

## Defining the 1956--1995 (historic) fleet
...
variables depth, date, and locality are available as model covariates.

*Insert details about omitting fishing events with more than five hours effort.*

## Defining the 1996--2007 (modern) fleet
...
take advantage of the higher resolution modern data that also includes
...
locality $j$, locality-year $k$, and vessel $l$ (for the modern dataset only);
$\phi$ represents the dispersion parameter;

The intercepts are allowed to vary from the overall intercept \(\alpha\) by
locality (\(\alpha^\mathrm{locality}_j\)), locality-year
(\(\alpha^\mathrm{locality-year}_k\)), and vessel
(\(\alpha^\mathrm{vessel}_l\)) (for the modern dataset only) with respective standardized deviations denoted by $\sigma$ parameters.
...
levels with the year set to the year of interest.

We chose the reference levels as the most frequent level of each predictor in
the positive-only data [@maunder2004a]. For example, we set the reference
month as the most common month observed in the dataset filtered for only tows
where the species was caught. This will not have on the shape of the final CPUE
index...

<!--copied! We estimate ﬁxed effects via maximum marginal likelihood while integrating across all random effects (see Table 1 for full list of parameters). The marginal likelihood is approximated using the Laplace approximation (Skaug and Fournier, 2006; Thorson et al., 2015) as implemented using template model builder (Kristensen et al., 2014).-->

We fit the models with the statistical software TMB via glmmTMB and use
standard errors ($\mathrm{SE}$) as calculated by TMB on $\log (\mu_t)$ via the
delta method. We then calculated the 95\% Wald confidence intervals as $\exp
(\mu_t \pm 1.96 \mathrm{SE}_t)$.
...
*TODO: write up the simulation paragraph*
