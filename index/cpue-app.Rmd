...
1. The two models are fit with separate links and so the coefficients cannot be
   combined.
1. It has shown to be insufficiently robust to TODO in some situations... REFs
1. Something else from Jim's recent paper REFs
...
density function has been introduced to the software TMB and can be fit
relatively quickly.
...
between the Poisson (if $p = 1$) and the Gamma distribution (if $p = 2$). In
fact, the Tweedie is alternatively referred to as the compound-Poisson-Gamma
distribution in this bounded case, although the compound-Poisson-Gamma
distribution can also refer to a true compound distribution in which the
Poisson and Gamma components can have their own means derived from their own
predictive models (REF). We cannot fit such a model here, with separate Poisson
and Gamma compounds --- sometimes referred to as a Poisson-link model (Thorson
REF) --- because we do not know the number of fish samples contributing to
the count or Poisson component.
...
coefficients, and $\mu_i$ represents the estimated probability of observing the
species in a tow.
