...
inner_join(est, se) %>%
  ...
  dplyr::select(Survey,Term, Estimate, Conf.low, Conf.high) %>% 
   knitr::kable(caption = "coefficients from the logistic regression maturity ogives", booktabs = TRUE, linesep = "", digits=2)
...
```{r mat-fifty-tab, results = 'asis'}
 ...
 #based on outputs from age-mat-coefs-table
  New <- inner_join(est, se) %>%
    mutate(.id = factor(.id, levels = names(surveys_list))) %>%
    rename(Survey = .id, Term = term, Estimate = estimate) %>%
    dplyr::select(Survey,Term, Estimate)

 #Get age at 50% maturity
 Survs <- c("SYN HS, SYN QCS, SYN WCVI", "SYN HS, SYN QCS", "SYN WCVI")
 a50table <- as.data.frame(matrix(nrow=6, ncol=3))
 colnames(a50table) <- c("Survey", "Sex", "Estimated age-at-50% maturity")
 ii <- 0

 for(i in seq_along(Survs)){
  Pars <- New %>%
    dplyr::filter(Survey==Survs[i])

  beta0 <- Pars$Estimate[which(Pars$Term=="intercept_male")]
  beta1 <- Pars$Estimate[which(Pars$Term=="slope_male")]
  beta2 <- Pars$Estimate[which(Pars$Term=="intercept_female")]
  beta3 <- Pars$Estimate[which(Pars$Term=="slope_female")]

   a50table[ii+1,1] <- Survs[i]
   a50table[ii+2,1] <- Survs[i]
   a50table[ii+1,2] <- "Female"
   a50table[ii+2,2] <- "Male"
   a50table[ii+1,3] <- -(log(1)+beta0+beta2)/(beta1 + beta3)
   a50table[ii+2,3] <- -(log(1)+beta0)/beta1
  ...
  a50table %>%
  as.data.frame() %>%
  knitr::kable(caption = "Estimates of age at 50 per cent maturity from the logistic regression maturity  ogives.",     booktabs = TRUE, linesep = "", digits=1)
...
## Age-length keys
For illustrative purposes, age-length keys (ALKs) were constructed using age and length data from the Hecate Strait, Queen Charlotte Sound and West Coast Vancouver Island Synoptic surveys for Pacific Cod. 
...
#Get annual obsered proportions at age
prop.age.obs <-d_survey_samples %>% 
  pcodALK::get_age_sample(surveys = c("SYN HS", "SYN QCS"),alkyr=c(2007, 2009, 2011)) %>%
  pcodALK::get_age_proportions() %>%
  write.csv("./Results/PropAgeObs_5ABCD.csv", row.names=T) 
  
prop.age.obs <-d_survey_samples %>% 
  pcodALK::get_age_sample(surveys = c("SYN WCVI"),alkyr=c(2006, 2008, 2010)) %>% 
  pcodALK::get_age_proportions() %>% 
  write.csv("./Results/PropAgeObs_3CD.csv", row.names=T)
...
for(i in seq_along(yrs)){
  pcod.age <- d_survey_samples %>% 
    pcodALK::get_age_sample(surveys = c("SYN HS", "SYN QCS"),alkyr=c(yrs[i])) %>% 
    pcodALK::get_ALK(bin=2) %>% 
    write.csv(paste0("./Results/ALK_5ABCD_",yrs[i],".csv"), row.names=T) 
  alk_list[[i]] <- pcod.age
}
