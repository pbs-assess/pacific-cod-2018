...
```{r, length-temp, echo=TRUE, eval=TRUE}
xx <- d$commercial_samples %>%
  dplyr::filter(!is.na(length))  #, sampling_desc == "UNSORTED"
...
*TO DO: Add query details for length data (survey and comm). Or maybe here is where description of gfplot goes. *

A total of `r total_survey_lengths` fish were measured in the three synoptic surveys ...
A summary of number of fish aged by year, survey and sex is provided in Table \@ref(tab:length-biosamples-surv) ...
Survey length-frequencies are shown in Figure \@ref(fig:length-plotfrequencies-surv) ...

Assessment of maturity is done by visual inspection of the gonads. Fish are assessed as being in one of six states: immature, maturing, mature (resting), mature, mature (spent) and mature (ripe). For the purposes of estimating proportion mature-at-age, fish were considered mature if they were assessed to be in any one of the latter four states. A summary of number of maturity records by year and survey is provided in Table \@ref(tab:mat-biosamples)

*TO DO: Tabulate criteria for commercial data below. Not really clear what it means. See main body and Appendix C of 2013 assessment and update with recent modifications if any.*

Length data from commercial vessels were extracted from the GFBio database, using
the criteria given below. Examination of the sample length frequencies
...
Summaries of number of fish aged by year, survey and sex are provided in Tables \@ref(tab:length-biosamples-com-5ABCD) and \@ref(tab:length-biosamples-com-3CD). Commercial length-frequencies are shown in Figure \@ref(fig:length-plotfrequencies-com).

*TO DO: Update gfplot code to not plot by sex. Commercial samples unsexed after 1999""
...
```{r length-plot-frequencies, fig.cap="Length-frequencies of Pacific Cod taken in the commercial trawl fishery. For clarity only lengths since 1996 are shown."}
...
*TO DO: add references below to bib and cite correctly.  Add info on length-stratification in surveys.*
...
*TO DO. Improve fig and table captions. Improve figure spacing.*
