...
Markov Chain Monte Carlo routines built into AD Model Builder [@fournier2012]. Posterior samples were drawn every 5,000 iterations from a chain of length 10 million, resulting in 2,000 posterior samples (the first 1,000 samples were dropped to allow for sufficient burn-in). We assessed consistency with chain convergence by visual inspection of trace plots and autocorrelation plots and through the potential scale reduction factor $\hat{R}$ and the effective sample size ($n_{eff}$).

The potential scale reduction statistic [@gelman1992] is a common metric used in Bayesian statistics to assess chain convergence [@gelman2014; @hobbs2015. It measures the the ratio of the average variance of MCMC samples within each chain to the variance of the samples across chains. In this case, we only had a single chain, but in following the advice of @gelman2014, we first divided the chain into two, effectively treating the first and second half as separate chains.

<!-- https://github.com/stan-dev/stan/blob/238faf42b556c007197c9faca20214ad9b37f0b2/src/docs/reference-manual/analysis.Rmd -->

Following the notation of @hobbs2015, we can calculate $\hat{R}$ as follows. For $j = 1, ..., m$ chains (two half chains here), we calculate the mean of the variances $w$ as:

$$
w = \frac { 1 } { m } \sum _ { j = 1 } ^ { m } \operatorname { var } \left( \theta_{ j } \right)
$$

where the mean of each chain \(\theta_{j}\) is defined as
$\theta_{j}=\frac{1}{K-1}\sum_{n=1}^{K}\left(\theta_{kj}-\overline{\theta}_{j}\right)^{2}$
and across $k = 1, ..., K$ iterations.

We then define the among-chain variance $b$ as:

$$
b = \frac { K } { m - 1 } \sum _ { j = 1 } ^ { m } \left( \overline { \theta } _ { j } - \overline { \overline { \theta } } \right) ^ { 2 }
$$

where
$\overline{\overline{\theta}}=\frac{1}{m}\sum_{j=1}^{m}\overline{\theta}_{j}$.
The variance of a stationary distribution of $\theta$ is then a weighted mean:

$$
\sigma _ { \theta } ^ { 2 } = \left( 1 - \frac { 1 } { K } \right) w + \frac { 1 } { K } b
$$

and we can define the potential scale reduction factor as:

$$
\hat { R } = \sqrt { \frac { \sigma _ { \theta } ^ { 2 } } { w } }.
$$

If $\hat{R}$ is large, it is indicative that we need to increase the number of samples. As $\hat{R}$ approaches 1.0, the chains are consistent with convergence. We ensured that $\hat{R}<1.05$ for all parameters.
...
Performance measures were calculated over a sequence of alternative 2018 projected catch levels and are based on a one-year projection to 2019.

Projected recruitment anomalies for the projection year were drawn randomly from a normal distribution, $\mathcal{N}\sim(0,\sigma_R)$.
...
Decision tables are designed to present probabilities of "undesirable" states under alternative 2018 projected catch levels.

For example, an undesirable biomass-based performance measure occurs when 2019 projected biomass is below the LRP, i.e., $B_{2019}/LRP < 1$. An undesirable fishing mortality-based performance measure occurs when projected 2014 fishing mortality is above the reference point, i.e., $F_{2018}/LRR > 1$.
...
The decision table for the Area 5ABCD Reference Case model is provided in Table XX.
