...
Suggested reference points and benchmarks for both areas are provided in Table \@ref(tab:suggested-ref-points). We do not currently present alternative reference points in this assessment (e.g., reference points based on $B_{0}$), but suggest these could be explored for these stocks, as they have been shown to perform well under simulated conditions [@forrest2018].
...
As for the previous stock assessment for Pacific Cod (@forrest2013), fixed variance components were used for the likelihood components for annual catch ($\sigma_C$) and average annual mean weight ($\sigma_W$). Because the model assumes observed catches are known without error, $\sigma_C$ was set to a small value, i.e., $\sigma_C$ = 0.05. As in @forrest2013, $\sigma_W$ = 0.2. Sensitivity tests are provided in Section \@ref(sensitivity-analyses).
...
> \textbf{Sc 1f.} Remove both CPUE indices from the model.
...
> \textbf{Sc 2a.} The mean of the QCS Synoptic survey was set to the same mean as for the HS Synoptic survey, i.e., mean $ln(q)$ = $ln(0.186)$.

> \textbf{Sc 2b.} A uniform prior was used for \textit{q} for both the QCS and HS Synoptic Surveys.
...
> \textbf{Sc 2a.} The sd of the WCVI Synoptic survey was set to the same sd as for the QCS and HS Synoptic surveys, i.e., sd = 0.3.

> \textbf{Sc 2b.} A uniform prior was used for \textit{q} for the WCVI Synoptic Surveys.
...
> \textbf{Sc 4a.} Use a uniform prior for steepness bounded between 0.21 and 0.999.

> \textbf{Sc 4b.} Use a beta prior for steepness with mean = 0.85 and sd = 0.15.
...
Another sensitivity analysis (Sc 5b) was done to illustrate the effects of reverting back to the von Bertalanffy growth parameters used in the previous Area 5CD assessment [@forrest2013]. The previous stock assessment used the von Bertalanffy parameters that @westrheim1996 reported for Area 3CD, i.e., $L_\\infty$ = 89.48 cm; $K_{VB}$ = 0.307 *y*^1^; and $t_0$ = -0.116 *y*. @forrest2013 also used length-weight parameters $a_{LW}$ = 7.38*e*-6 and $b_{LW}$ = 3.0963. The delay-difference model in scenario 5b used Ford-Walford parameters $W_k$, $\\alpha_g$ and $\\rho_g$ derived from these previous growth parameters. In summary, the two new scenarios are:

> \textbf{Sc 5a.} Set $k = 3 y$ and update weight at recruitment $W_k$ and Ford-Walford parameters accordingly.

> \textbf{Sc 5b.} Run the model using the von Bertalanffy growth parameters that were used in the previous stock assessment [@forrest2013].
...
Fits to the average annual mean weight in the commercial fishery for Scenarios 7a and 7b are shown in Figures \@ref(fig:fig-sens7-meanweightfit-5abcd) and \@ref(fig:fig-sens14-meanweightfit-3cd).
