...
In the current assessment, these reference points are incorporated into the decision tables (Section (\@ref(projections-and-decision-tables)), where projected probabilities of breaching the reference points are reported, i.e., they are not incorporated into a formal harvest control rule.
...
The model appeared to have converged (Figure \@ref(fig:fig-base-mcmc-trace-5abcd)) and posterior autocorrelation was minor for most parameters (Figure \@ref(fig:fig-base-mcmc-autocor-5abcd)). The value of $\hat{R}$ was 1.00 for all parameters (Table \@ref(tab:tab-param-est-table-5abcd)).
...
Model fit to the total commercial catch is shown in Figure \@ref(fig:fig-base-catch-fit-5abcd). Model fits are almost perfect due to the setting $\sigma_C$ = 0.05 in the objective function (Appendix \@ref(delay-difference-model)). This was done so that the model was essentially conditioned on the catch data, which was assumed to be known with very little error, even though pre-1996 discard rates are very uncertain.
...
The model appeared to have converged (Figure \@ref(fig:fig-base-mcmc-trace-3cd)) and posterior autocorrelation was minor for most parameters (Figure \@ref(fig:fig-base-mcmc-autocor-3cd)).The value of $\hat{R}$ was 1.00 for all parameters (Table \@ref(tab:tab-param-est-table-3cd)).
...
Posterior probability distributions of estimated parameters are provided in Figure \@ref(fig:fig-base-mcmc-priors-posts-3cd) and Table \@ref(tab:tab-param-est-table-3cd). The median, 2.5^th^ percentile and 97.5^th^ percentile posterior parameter estimates, and maximum posterior density (MPD) estimates, are provided in Table \@ref(tab:tab-param-est-table-3cd). As for Area 5ABCD, with the exception of steepness, the posterior estimates did not appear to be strongly influenced by the prior probability distributions. The posterior probability distribution for steepness, however, was very similar to the prior probability distribution (Figure \@ref(fig:fig-base-mcmc-priors-posts-3cd)), implying that there is little information about this parameter in the available data. Sensitivity to the prior distribution assumed for steepness is tested in Section (\@ref(sensitivity-analyses)).
...
Survey scaling parameters ($q_{k}$) were strongly positively correlated with each other (Figure \@ref(fig:fig-base-mcmc-pairs-3cd)). As for Area 5ABCD, the survey scaling parameters were negatively correlated with $M$,implying that there is limited information in the data to distinguish between a small productive population or a larger, less productive population (Figure \@ref(fig:fig-base-mcmc-pairs-3cd)). Interestingly, $R_0$ was positively correlated with $M$.As for the QC Sound synoptic survey, the estimated posterior distribution for the WCVI synoptic survey scaling parameter was outside of the prior (Figure \@ref(fig:fig-base-mcmc-priors-posts-3cd)). Estimates of reference points and benchmarks are provided in Table \@ref(tab:tab-ref-points-table-3cd).
...
> **Sc 2a.** The mean of the QCS Synoptic Survey was set to the same mean as for the HS Synoptic Survey, and was then pro-rated according to relative areas, i.e., mean $\ln(q)$ = $\ln(0.1869 * 0.65)$.
...
Performance measures were calculated over a sequence of alternative 2018 projected catch levels and are based on a one-year projection to 2019 (Figures \@ref(fig:fig-base-biomass-5abcd-proj) and \@ref(fig:fig-base-biomass-3cd-proj)). Uncertainty enters the projections through recruitment anomalies for the projection year, which were drawn randomly from a normal distribution, $\mathcal{N}(0,\sigma_R^2)$. The following performance measures were evaluated:
...
- $USR$ (Upper Stock Reference) is the historical mean of the biomass estimates from 1956--2004 (both areas).
- $LRP$ (Limit Reference Point) is the lowest biomass from which the stock recovered above the USR. For Area 5ABCD this is
  the biomass estimate for 1971. For Area 3CD it is the biomass estimate for 1986.
- $LRR$ (Limit Reference Rate) is the Fishing mortality rate for the same years as the LRP.

For more information, see Section \@ref(reference-points)
...
The probability of the 2019 biomass being below the 2018 biomass ranged from 9% under no 2018 catch to 89% under a catch of 3,000 mt.

The probability of 2019 biomass being below the LRP ranged from 88% to 98% over the range of catch levels considered.There was a 100% probability of the 2019 biomass being below the USR over the entire range of catch levels considered.
...
Under a 2018 catch level of 900 mt, close to the 2017 TAC, there is an estimated 92% probability that the 2019 biomass will be below the LRP and a 100% probability that the 2019 biomass will be below the USR.
...
The probability of the 2019 biomass being below the 2018 biomass ranged from 84% under no 2018 catch to 92% under a catch of 1,500 mt.

The probability of 2019 biomass being below the proposed LRP was 0.0% over the entire range of catch levels considered. However, the probability of 2019 biomass being below the USR ranged from 98% to 99% over the entire range of catch levels considered.

The probability of the 2018 fishing mortality being greater than the 2017 fishing mortality ranged from 0.0% to 100%. The probability of the 2018 fishing mortality being greater than the LRR also ranged from 0.0% to 100%.

Under a 2018 catch level of 500 mt, the 2017 TAC, there is estimated 0.0% probability that the 2019 biomass will be below the LRP and a 98% probability that the 2019 biomass will be below the USR.

## MODEL-AVERAGED DECISION TABLES

We selected four sensitivity scenarios (plus the Reference Case) for each area to include in a model-averaged decision table. The sensitivity scenarios below were chosen because they produced plausible results that diverged from the Reference Case models. Inclusion of these posterior samples in the decision table may therefore better bracket the uncertainty due to the different assumptions captured in the sensitivity scenarios.

The vectors of 1,000 burned-in posterior samples from each of the five models, for each performance measure, under each alternative 2018 catch level, were combined into a single vector of 5,000 samples and probabilities of performance measures were calculated from the combined samples.

A similar approach was used in the 2011 assessment of Pacific Hake [@stewart2011], where two alternative stock assessment models were judged by the Pacific Fishery Management Council's Scientific and Statistical Committee to be equally plausible. A similar "ensemble" aproach is used in the annual assessments of Pacific Halibut, where four structurally distinct models are used [@stewart2016].
...
1. Reference Case;
2. the sensitivity case without commercial CPUE data (Sc 1f);
3. the sensitivity case with prior for QCS $q$: $\ln(q)$ = $\ln(0.1869 * 0.65)$ (Sc 2a);
4. the sensitivity case with prior for ln($M$): $\mathcal{N}\sim(0.4,0.2)$ (Sc 3b);
5. the sensitivity case with $\sigma_0 = 0.15$ (Sc 6b).

The "Model-Averaged" decision table probabilities for Area 5ABCD are presented in Table \@ref(tab:tab-decision-avg-5abcd).

The probability of the 2019 biomass being below the 2018 biomass ranged from 10% under no 2018 catch to 89% under a catch of 3,000 mt.

The probability of 2019 biomass being below the LRP ranged from 82% to 97% over the range of catch levels considered.There was a 100% probability of the 2019 biomass being below the USR over the entire range of catch levels considered.

The probability of the 2018 fishing mortality being greater than the 2017 fishing mortality ranged from 0.0% to 100%. The probability of the 2018 fishing mortality being greater than the LRR also ranged from 0.0% to 100%.

Under a 2018 catch level of 900 mt, close to the 2017 TAC, there is an estimated 89% probability that the 2019 biomass will be below the LRR and a 100% probability that the 2019 biomass will be below the USR.

### Area 3CD

For Area 3CD, we present an alternative decision table with probabilities calculated from combined posterior estimates of performance measures from the following five models:

1. Reference Case;
2. the sensitivity case without commercial CPUE data (Sc 1f);
3. the sensitivity case with the SD of the prior for the WCVI Synoptic Survey = 0.448 (Sc 2a);
4. the sensitivity case with prior for ln($M$): $\mathcal{N}\sim(0.4,0.2)$ (Sc 3b);
5. the sensitivity case with $\sigma_0 = 0.15$ (Sc 6b).

The "Model-Averaged" decision table probabilities for Area 3CD are presented in Table \@ref(tab:tab-decision-avg-3cd).

For most performance measures, probabilities were similar to those in the Reference Case model.

The probability of the 2019 biomass being below the 2018 biomass ranged from 75% under no 2018 catch to 91% under a catch of 1,500 mt.

The probability of 2019 biomass being below the LRP was 18% over the range of catch levels considered.The probability of 2019 biomass being below the USR ranged from 98% to 99% over the range of catch levels considered.

The probability of the 2018 fishing mortality being greater than the 2017 fishing mortality was 100% across the range of catch levels considered. The probability of the 2018 fishing mortality being greater than the LRR ranged from 0.0% to 98%.

Under a 2018 catch level of 500 mt, close to the 2017 TAC, there is an estimated 18% probability that the 2019 biomass will be below the LRP and a 99% probability that the 2019 biomass will be below the USR.

## SUMMARY

We presented the alternative, model averaged decision tables for Areas 5ABCD and 3CD in an attempt to more comprehensively incorporate substantial structural uncertainty in the assessments into advice for fishery managers and stakeholders.
...
- The effects of the assumption of constant selectivity in the trawl fishery;
- The effect of the assumption that recruitment to the fishery, surveys and the spawning biomass is knife-edged at age 2 years;
...
- The impact of change in onboard observer coverage and representativeness of length samples from the commercial catch.
...
the historical commercial catch per unit effort data. Her work on this important data set for this
assessment made it possible to have a fully reproducible analysis and will be
helpful in future assessments for other stocks. More generally, her
help with the gfplot R package, and its carefully constructed SQL queries, made
the data extraction for this assessment seamless. We thank Norm Olsen for his advice on the historical commercial catch per unit effort reconstruction. We also thank Greg Workman and
Maria Surry (Pacific Biological Station) for their support and advice with the
preparation for this assessment.We thank Bruce Turris and Brian Mose (Canadian Groundfish Research and Conservation Society) for their advice to the authors.
