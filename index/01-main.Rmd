...
*TO DO: Find out why Appendix D (model-description-app.Rmd) is being referred to as a subsection of Appendix C: Appendix C.6.1.1*
...
Markov Chain Monte Carlo routines built into AD Model Builder [@fournier2012]. Posterior samples were drawn systematically every 5,000 iterations from a chain of length 10 million, resulting in 2,000 posterior samples (the first 1,000 samples were dropped to allow for sufficient burn-in). Convergence was diagnosed using visual inspection of trace plots and visual examination of autocorrelation in posterior
chains.
...
*TO DO: Paul, please provide a more accurate description of why the swept area analyses for HSMAS and Triennial could not be expected to scale up to absolute abundance. Anyone, improvements to the text below are welcome*
...
These priors reflected ignorance of the scale of the survey index relative to the population. For the Hecate Strait Multispecies Assemblage Survey and NMFS Triennial Survey, the uniform priors also reflected the fact that the swept area analyses could not be expected to scale up to an absolute index of abundance because the survey designs were not random-stratified surveys intended to represent the entire habitat of Pacific Cod. The commercial CPUE indices were represented standardized catch rates with an unknown relationship to population abundance.
...
The choice of prior probability distribution for survey scaling parameters has the potential for large impacts on stock assessment outcomes. We therefore present a number of sensitivity analyses in Section \@ref(sensitivity-analyses)

### Survey scaling parameters for Area 5ABCD

We attempted to use informative prior probability distributions for synoptic surveys based on median posterior estimates of *q* from the most recent stock assessment for Rock Sole [@holt2016]. Rock sole is a flatfish species occupying a similar (although slightly shallower) depth range to Pacific Cod. @holt2016 obtained median posterior estimates of *q* = 0.6280 for Area 5AB and *q* = 0.1869 for Area 5CD. The CV of their estimates was around 0.3. For our combined Area 5ABCD assessment we pro-rated these values according to the relative areas of habitat in Areas 5AB and 5CD. Adding up the areas of all the surveyed depth strata in each area gave a ratio of approximately 0.65:0.35 (5AB:5CD). Therefore the following values were used as means for the normal prior probability distributions for $ln(q_{k})$, for
...
*TO DO: RF to add commentary to sensitivities*
...
*TO DO: Sean can you please correct this text as needed and add correct equation references*

The reference case models were fit to historical and modern commercial CPUE indices that included a year-location interaction as random effect (see Appendix \@ref(commercial-cpue-standardization)). Two sensitivity analyses are shown to illustrate the effect of removing the year-location interaction term from both historical and modern indices (i.e., fitting the delay-difference model to historical and modern CPUE indices calculated using equations without the interaction term:

> **Sc 1b.** Use equations without the interaction term to create CPUE
  ...
  difference model (Equation TX.X).

> **Sc 1c.** Use equations without the interaction term to create CPUE
  indices for historical and modern periods AND Double the annual CVs resulting
  from the analysis. Input the doubled CVs as weighting terms in objective function of delay
  difference model (Equation TX.X).

Sc 1c was done because the annual CVs of the CPUE indices were very small (less than 0.1 in some years). Annual CVs were doubled to address concerns that the objective function of the delay-difference model may have been giving the CPUE indices too much weight with the CVs in Sc 1b.
...
index includes year-location interaction term as a random effect.
...
Resulting posterior estimates of biomass, recruitment and fishing mortality compared to the Reference Case models are shown in Figures \@ref(fig:fig-sens2-biomass-5abcd) to \@ref(fig:fig-sens2-f-5abcd) for Area 5ABCD and Figures \@ref(fig:fig-sens9-biomass-3cd) to \@ref(fig:fig-sens9-f-3cd) for Area 3CD.
...
For Area 5ABCD, respective median posterior estimates of *M* for Scenarios 3a, 3b and 3c were 0.323 y^-1^, 0.347 y^-1^ and 0.298 y^-1^. For Area 3CD, respective median posterior estimates of *M* for Scenarios 3a, 3b and 3c were 0.392 y^-1^, 0.391 y^-1^ and 0.398 y^-1^.
...
For Area 5ABCD, respective median posterior estimates of *h* for Scenarios 4a and 4b were 0.479 and 0.883. For Area 3CD, respective median posterior estimates of *h* for Scenarios 4a and 4b were 0.497 and 0.889.

Biomass and parameter estimates show that, while differences in estimated *h* among the Reference Cases and two scenarios, there was relatively little effect of the prior probability distribution for steepness on posterior estimates of biomass.
...
Another sensitivity analysis (Sc 5b) was done to illustrate the effects of reverting back to the von Bertalanffy growth parameters used in the previous Area 5CD assessment [@forrest2013]. The previous stock assessment used the von Bertalanffy parameters that @westrheim1996 reported for Area 3CD, i.e., $L_{inf}$ = 89.48 cm; $K_{VB}$ = 0.307 *y*^1^; and $t_0$ = -0.116 *y*. @forrest2013 also used length-weight parameters $a_{LW}$ = 7.38*e*-6 and $b_{LW}$ = 3.0963. The delay-difference model in scenario 5b used Ford-Walford parameters $W_{k}$, $\alpha_g$ and $\rho_g$ derived from these previous growth parameters. In summary, the two new scenarios are:
...
Resulting posterior estimates of biomass, recruitment and fishing mortality compared to the Reference Case models are shown in Figures \@ref(fig:fig-sens6-biomass-5abcd) to \@ref(fig:fig-sens6-f-5abcd) for Area 5ABCD and Figures \@ref(fig:fig-sens13-biomass-3cd) to \@ref(fig:fig-sens13-f-3cd) for Area 3CD. Fits to the indices of abundance are shown in Figures \@ref(fig:fig-sens6-index-cpue-post1995-5abcd) and \@ref(fig:fig-sens13-index-triennial-3cd). Resulting parameter estimates are provided in Tables \@ref(tab:tab-param-table-sens6a-5abcd) to \@ref(tab:tab-param-table-sens6c-5abcd) for Area 5ABCD and Tables \@ref(tab:tab-param-table-sens13a-3cd) to \@ref(tab:tab-param-table-sens13c-3cd) for Area 3CD.
...
Resulting posterior estimates of biomass, recruitment and fishing mortality compared to the Reference Case models are shown in Figures \@ref(fig:fig-sens7-biomass-5abcd) to \@ref(fig:fig-sens7-f-5abcd) for Area 5ABCD and Figures \@ref(fig:fig-sens14-biomass-3cd) to \@ref(fig:fig-sens14-f-3cd) for Area 3CD. Fits to the average annual mean weight in the commercial fishery for Scenarios 7a and 7b are shown in Figures \@ref(fig:fig-sens7-meanweightfit-5abcd) and \@ref(fig:fig-sens14-meanweightfit-3cd).

# DECISION TABLES

*TO DO: Update. This is 2014 text. Fix equations and dynamic references*

Performance measures were calculated over a sequence of alternative 2019 projected catch levels and are based on two-year projections to 2020.

Projected recruitment anomalies for the projection period (2019-2020) were drawn randomly from a normal distribution, $\mathcal{N}\sim(0,\sigma_R)$.

Reference points and performance measures are listed in Tables \@ref(tab:tab-ref-points-table-5abcd) and \@ref(tab:tab-ref-points-table-3cd). 

Decision tables are designed to present probabilities of "undesirable" states under alternative 2019 projected catch levels.

For example, an undesirable biomass-based performance measure occurs when 2020 projected biomass is below the LRP, i.e., $B_{2020}/LRP < 1$. An undesirable fishing mortality-based performance measure occurs when projected 2014 fishing mortality is above the reference point, i.e., $F_{2019}/LRR > 1$.
...
*TO DO. Update with results*

The decision table for the Area 5CD Reference Case model is provided in Table XX.
