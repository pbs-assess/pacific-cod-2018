...
We selected four sensitivity scenarios (plus the Reference Case) for each area to include in a model-averaged decision table. The sensitivity scenarios below were chosen because they produced plausible results that diverged from the Reference Case models. Inclusion of these posterior samples in the decision table may therefore better bracket the uncertainty due to the different assumptions captured in the sensitivity scenarios.However, please note that the choices of sensitivity cases included in the model-averaged tables should be considered placeholders for discussion and agreement at the upcoming regional peer review meeting.
...
3. the sensitivity case with prior for QCS $q$: mean $\ln(q)$ = $\ln(0.1869 \cdot 0.65)$ (Sc 2a);
...
4. the sensitivity case with prior for ln($M$): $\mathcal{N}\sim(0.4,0.1^2)$ (Sc 3b);
5. the sensitivity case with $\sigma_0 = 0.15$ (Sc 6b).
