...
get_par_df(vb_fits, 1) %>%
  ...
  footnote(alphabet = c("Note that these values differ slightly from those used in the models and described in the model appendix. The values in this table were run with extra chains and for more iterations with a higher Stan 'adapt delta' argument to more precisely explore the joint posterior. The models will be updated with these values."))
```
