...
The last assessment for Pacific Cod in QCS and Hecate Strait used a delay-difference model [@forrest2013]. Essentially a delay-difference model is a collapsed age-structured model, subject to certain assumptions
...
- Knife edge selectivity, where all fish aged $k$ *y* and older are equally
  vulnerable to the fishing gear, and knife-edged maturity at age $k$ *y*; and
...
- three penalty functions that:
  - constrain the estimates of annual recruitment to conform to a
    Beverton-Holt stock-recruit function (Eq. \@ref(eq:ddrt));
  - weakly constrain the log recruitment deviations to a normal distribution ($\sim N(0,2)$);
  - very weakly constrain estimates of log fishing mortality to a normal
...
The abundance indices (Appendices \@ref(fishery-independent-indices-of-abundance) and \@ref(commercial-cpue-standardization)) were treated as relative abundance indices, assumed to be directly proportional to the biomass with lognormal errors. The survey scaling (catchability) parameter for each survey $q_j$ was treated as an uncertain parameter, with the conditional maximum posterior density (MPD) estimate of $q_j$ used in the objective function (Eq. \@ref(eq(ddljt)), where the parameter
$j_z$ represents the maximum likelihood estimate of $ln(j_q)$, conditional on other model parameters, with $n_j$ the number of observations in index $j$ [@walters1994; Eqs. \@ref(eq(ddzjt)--\@ref(eq(dddjt)].
...
model.param.desc.table(cap = paste0("List of parameters for the delay-difference model.",
                                    ...
                                    "in bold type. See Tables XX-XX for derivation of ",
                                    "other parameters."))
```
...
\begin{equation}
\left\{ \begin{array}{l}{N_{2,1}} = {R_{Avg}}{e^{{\omega _1}}}\quad \quad \quad \quad \quad \quad \quad \quad a = 2\\{N_{a,1}} = \left( {{R_{AvgInit}}{e^{\omega Ini{t_a}}}} \right){e^{ - M\left( {a - 2} \right)}} \quad 2 < a < A\\{N_{A,1}} = \frac{{\left( {{R_{AvgInit}}{e^{\omega Ini{t_A}}}} \right){e^{ - M\left( {A - 2} \right)}}}}{{\left( {1 - {e^{ - M}}} \right)}}\quad \quad a = A\end{array} \right\}
(\#eq:ddn1)
\end{equation}
...
\begin{equation}
{R_t} = {R_{Avg}}{e^{{\omega _t} - \frac{{\sigma _R^2}}{2}}}
(\#eq:ddrt)
\end{equation}
