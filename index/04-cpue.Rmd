...
100*sum(is.na(raw_dat$locality_desc)) / nrow(raw_dat)
...
out <- raw_dat %>% group_by(year, area, trip_id, fishing_event_id) %>%
  summarize(
    ...
    month = month[[1]],
    locality = paste0("-", locality_desc[[1]]) # - is for formatting for coef plotting
  ) %>% ungroup()
...
plot_pcod_dist <- function(dat, y, ylab = "", title = "") {
  ggplot(dat, aes_string("as.factor(year)", y = y)) + geom_boxplot() + 
    ...
    scale_x_discrete(breaks = seq(1950, 2020, 2)) +
    theme_pbs() +
...
out <- out %>% 
  filter(year < 1991 | year >= 1991 & hours_fished <= 12)
...
out_trip <- out %>% group_by(year, area, trip_id) %>%
  summarize(
    ...
    month = rev(names(table(month)))[[1]],
    locality = rev(names(table(locality)))[[1]],
    n_month = length(unique(month)),
    n_locality = length(unique(locality))
  ) %>% ungroup()

plot_pcod_dist(filter(out_trip, spp_catch > 0), y = "spp_catch/hours_fished", 
  ylab = "CPUE (kg/hr)", title = "CPUE, positive trips")
plot_pcod_dist(filter(out_trip, spp_catch > 0), y = "spp_catch", 
  ylab = "Catch (kg)", title = "Catch, positive trips")
plot_pcod_dist(filter(out_trip, spp_catch > 0), y = "hours_fished", 
  ylab = "Effort (hours)", title = "Effort, positive trips")
plot_pcod_dist(out_trip, y = "hours_fished", 
  ylab = "Effort (hours)", title = "Effort, all trips")
plot_pcod_dist(filter(out_trip, spp_catch > 0), y = "exp(mean_log_depth)", 
  ylab = "Mean depth", title = "Mean depth, positive trips")
plot_pcod_dist(out_trip, y = "exp(mean_log_depth)", 
  ylab = "Mean depth", title = "Mean depth, all trips")
...
group_by(out_trip, area, locality) %>%
