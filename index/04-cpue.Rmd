...
mean_log_pos_depth <- mean(log(filter(dat_, pos_catch == 1)$depth))
dat_$log_depth_scaled <- (dat_$depth - mean_log_pos_depth) / (2 * sd(dat_$depth))

m_re <- gfplot:::fit_cpue_index_re(dat_)

# pos_dat <- filter(dat_, pos_catch == 1)
# 
# # Random effect version:
# fct_to_tmb_num <- function(x) {
#   as.numeric(as.factor(as.character(x))) - 1
# }
# bin_vess <- fct_to_tmb_num(dat_$month)
# pos_vess <- fct_to_tmb_num(filter(dat_, pos_catch == 1)$month)
# bin_loc <- fct_to_tmb_num(dat_$locality)
# pos_loc <- fct_to_tmb_num(filter(dat_, pos_catch == 1)$locality)
# 
# base_depth <- "050"
...
#   log_depth_scaled + I(log_depth_scaled^2)
...
#   log_depth_scaled + I(log_depth_scaled^2)
#   
# mm1 <- model.matrix(formula_binomial, data = dat_)
# mm2 <- model.matrix(formula_lognormal, data = pos_dat)
# mm_pred1 <- make_pred_mm(mm1, years = unique(pos_dat$year_factor))
# mm_pred2 <- make_pred_mm(mm2, years = unique(dat_$year_factor))
# 
# # find good fixed-effect starting values from GLMs/LMs:
# m_bin <- speedglm::speedglm(formula_binomial, data = dat_,
#   family = binomial(link = "logit"))
# m_pos <- lm(formula_lognormal, data = filter(dat_, pos_catch == 1))
# 
# library(TMB)
# 
...
# 
#     log_sigma = log(1.5),
#     log_sigma_zk1 = log(0.2),
#     log_sigma_zk2 = log(0.2),
#     log_sigma_zg1 = log(0.2),
#     log_sigma_zg2 = log(0.2)
#   ),
...
object <- list()
object$model <- obj
object$sdreport <- sdreport(obj)
object$mm_bin <- mm1
object$mm_pos <- mm2
object$years <- sort(unique(dat_$year))
# 
# # gfplot::tidy_cpue_index_coefs(object, 
# #   remove = c("z1_k", "z1_g", "z2_g", "z2_k", "log_sigma_zk1", "log_sigma_zk2", "log_sigma_zg1", "log_sigma_zg2")) %>% 
# #   plot_cpue_index()

predictions_re <- predict_cpue_index(object)
```
...
bind_rows(data.frame(cpue_stand, type = "glm"), 
  data.frame(predictions_re, type = "glmm", area = "5CD")) %>%
  filter(area == "5CD") %>%
