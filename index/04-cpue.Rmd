...
```{r cpue-plot-effort-fe, fig.cap="The distribution of fishing effort by year through time. Note that prior to 1991 the fishing events are rolled up to a trip level. The horizontal red line illustrates the six hour threshold that was used to remove erroneous fishing events in 1991 and after."}
...
TODO: I can plot out the coefficients but I haven't adopted the code yet to work with the random interceptor version. 
...
```{r, fig.cap="The unconditional combined estimate of standardized CPUE (colours) compared with the unstandardized CPUE time series (black). The first 3 columns illustrate the effect of the individual predictors or random intercepts and the last column combines all 3 elements into a single model."}
