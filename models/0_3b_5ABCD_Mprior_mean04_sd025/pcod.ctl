...
-0.9162907 -2.302585   0.     1  1    -0.9162907  0.25     # log.m log(0.4)=-0.9162907  log(0.596)=-0.5175146      log(0.3) = -1.203973 log(0.5) = -0.6931472
 8.9        1.0       12.     -1  0     1.0       12.      # log_avgrec
 9.54       1.0       12.     -1  0     1.0       12.      # log_recinit
 0.058824   0.01       0.999 -3  3     3.0       12.      # rho Pcod: rho and varphi fixed to give sig=0.2 and tau=0.8.
 1.470588   0.01     150.    -2  4     7.49836    5.78354 # kappa (precision) FOR P COD, VARPHI AND RHO ARE FIXED TO make average ratio of sds from the two surveys the same as 2005
## ------------------------------------------------------------------------- ##
##
## ------------------------------------------------------------------------- ##
...
## ------------------------------------------------------------------------- ##
  6      6      6      6     6      6     # 1 -selectivity type ivector(isel_type) for gear
  1.5    1.5    1.5    1.5  1.5    1.5    # 2 -Age/length at 50% selectivity (logistic)
  0.0001 0.0001 0.0001 0.0001 0.0001 0.0001 # 3 -STD at 50% selectivity (logistic)
  0      0      0      0   0  0   # 4 -No. of age nodes for each gear (0=ignore)
  0      0      0      0   0  0  # 5 -No. of year nodes for 2d spline(0=ignore)
 -1     -1     -1     -1    -1    -1   # 6 -Phase of estimation (-1 for fixed) If neg number, it reflects a mirroring of another gear's selectivity.
150.0  200.0  200.0  200.0  200.0  200.0    # 7 -Penalty wt for 2nd differences w=1/(2*sig^2)
 50.0  200.0  200.0  200.0  200.0  200.0    # 8 -Penalty wt for dome-shaped w=1/(2*sig^2)
...
## ------------------------------------------------------------------------- ##
5            # -number of surveys (nits)
0    1   1 0 0   # -prior type (see legend above)
0.   -0.896  -2.727   0.  0.  # -prior log(mean)
1.  0.3 0.3 1.0 1.0 # -prior sd
## ------------------------------------------------------------------------- ##
##
