...
# ANALYSE DES DONNÉES BIOLOGIQUES {#app:analysis-of-biological-data}
...
colnames(biotable) <- c(latex.mlc(en2fr("Survey", translate = french, allow_missing = TRUE)),
                     latex.mlc(en2fr("Year", translate = french, allow_missing = TRUE)),
                     latex.mlc(en2fr(c("N male"), translate = french, allow_missing = TRUE)),
                     latex.mlc(en2fr(c("N female"), translate = french, allow_missing = TRUE)),
                     latex.mlc(en2fr(c("N unsexed"), translate = french, allow_missing = TRUE)),
                     latex.mlc(en2fr(c("Max", "male", "(cm)"), translate = french, allow_missing = TRUE, case="lower")),
                     latex.mlc(en2fr(c("Max", "female", "(cm)"), translate = french, allow_missing = TRUE, case="lower")),
                     latex.mlc(en2fr(c("Max", "unsexed", "(cm)"), translate = french, allow_missing = TRUE, case="lower")))
                     
  
  knitr::kable(biotable, caption = "Nombre (N) de mesures de longueur prises lors des relevés synoptiques effectués dans le détroit d’Hécate (SYN HS), le bassin de la Reine-Charlotte (SYN QCS) et sur la côte ouest de l’île de Vancouver (SYN WCVI)", booktabs = TRUE, linesep = "", format = "pandoc")%>%
...
```{r length-plotfrequencies-surv, fig.cap="Fréquences de longueur de la morue du Pacifique prises lors des relevés synoptiques effectués dans le détroit d’Hécate (SYN HS), le bassin de la Reine-Charlotte (SYN QCS) et sur la côte ouest de l’île de Vancouver (SYN WCVI). Il convient de noter que seuls les spécimens mâles et femelles sont représentés sur le graphique."}
 ...
 x %>%
  ...
  plot_lengths()+
  xlab(paste(en2fr("Length",translate=french,allow_missing=TRUE), "(cm)")) +
  ylab(paste(en2fr("Relative",translate=french,allow_missing=TRUE),
             en2fr("length",translate=french,allow_missing=TRUE,case="lower"),
             en2fr("frequency", translate=french, allow_missing=TRUE,case="lower")))
...
tidy_ages_raw(x,
  ...
  plot_ages(
    ...
    max_size = 12, french=french
  ) +
  scale_x_continuous(breaks = 2006:2011) +
  scale_y_continuous(breaks = seq(1, 10)) +
  ylab(paste(en2fr("Age",translate=french,allow_missing=TRUE), 
             paste0("(",
                    en2fr("years",translate=french,allow_missing=TRUE,case="lower"),
                    ")")))+
  ggtitle("") +
  ylim(0, NA)
```
...
tidy_age_precision(d$age_precision, ageing_method_codes = 6) %>%
  ...
  xlab(en2fr("Primary age (years)", translate=french, allow_missing=TRUE))+
  ylab(en2fr("Secondary age (years)", translate=french, allow_missing=TRUE))+
  ggtitle("")
```
...
get_par_df(vb_fits, 1) %>%
  ...
  mutate(Sex = gsub("^male$", "Mâle", Sex)) %>%
  mutate(Sex = gsub("^female$", "Femelle", Sex)) %>%
  mutate(Sex = gsub("^all$", "Mâle et Femelle", Sex)) %>%
...
vb_figs <- map(vb_fits, ~ plot_vb(
  object_all = .x[[1]],
  col = c(All = "grey30"), lab_x = 0.38, french=french) +
    ggtitle(paste(unique(.x[[1]]$data$survey_abbrev), collapse = ", ")) +
     xlab(paste(en2fr("Age",translate=french,allow_missing=TRUE),
             paste0("(",
                    en2fr("years",translate=french,allow_missing=TRUE,case="lower"),
                    ")"))) +
    ylab(paste(en2fr("length",translate=french,allow_missing=TRUE), "(cm)"))+
    guides(colour = FALSE)
)
...
lw_figs <- map(lw_fits, ~ gfplot::plot_length_weight(
  object_all = .x[[1]],
      col = c(All = "grey30"), lab_x_gap = 0.45, french=french) +
    ggtitle(paste(unique(.x[[1]]$data$survey_abbrev), collapse = ", ")) +
    guides(colour = FALSE)+
    ylab(paste(en2fr("Weight",translate=french,allow_missing=TRUE), 
             paste0("(",
                    en2fr("kg",translate=french,allow_missing=TRUE,case="lower"),
                    ")")))+
     xlab(paste(en2fr("Length",translate=french,allow_missing=TRUE), 
             paste0("(",
                    en2fr("cm",translate=french,allow_missing=TRUE,case="lower"),
                    ")")))
  )
...
se <- group_by(mat_coefs, .id) %>%
  ...
  mutate(term = gsub("^intercept_male$", "Ordonnée à l’origine mâle: $\\\\beta_0$", term)) %>%
  mutate(term = gsub("^intercept_female$", "Ordonnée à l’origine femelle: $\\\\beta_0 + \\\\beta_2$", term)) %>%
  mutate(term = gsub("^slope_female$", "Pente femelle: $\\\\beta_1 + \\\\beta_3$", term)) %>%
  mutate(term = gsub("^slope_male$", "Pente mâle: $\\\\beta_1$", term)) %>%
  mutate(Conf.low = estimate - 1.96 * std.error, Conf.high = estimate + 1.96 * std.error) %>%
...
plyr::ldply(mat_fits, function(.x)
  ...
  melt() %>%
  mutate(.id = factor(.id, levels = names(surveys_list))) %>%
  mutate(variable = gsub("Male", "Mâle", variable)) %>%
  mutate(variable = gsub("Female", "Femelle", variable)) %>%
...
mat_figs <- map(mat_fits, ~ plot_mat_ogive(.x, french=french) +
    coord_cartesian(xlim = c(0, 8.2), ylim = c(0, 1), expand = FALSE)+
      ylab(en2fr("Probability mature", translate=french,allow_missing=TRUE))+
      xlab(paste(en2fr("Age",translate=french,allow_missing=TRUE), 
             paste0("(",
                    en2fr("years",translate=french,allow_missing=TRUE,case="lower"),
                    ")")))
...
```{r age-mat-coefs, fig.pos = "H", fig.cap = "Coefficients réorganisés à partir des ogives de la maturité par régression logistique (p. ex. Ordonnée à l’origine femelle = $\\beta_0 + \\beta_2$). Les paramètres sont dans l’espace logit ou des probabilités logarithmiques. L’interception fait référence aux probabilités logarithmiques de la maturité pour un poisson théorique de 0 an.", fig.width=6, out.width="4.5in"}
#based on outputs from age-mat-coefs-table
inner_join(est, se) %>%
  mutate(conf.low = estimate - 1.96 * std.error) %>%
  mutate(conf.high = estimate + 1.96 * std.error) %>%
  mutate(.id = factor(.id, levels = names(surveys_list))) %>%
  mutate(term = gsub("intercept_male","Ordonnée à l’origine mâle",term)) %>%
  mutate(term = gsub("intercept_female","Ordonnée à l’origine femelle",term)) %>%
  mutate(term = gsub("slope_male","Pente mâle",term)) %>%
  mutate(term = gsub("slope_female","Pente femelle",term)) %>%
  ggplot(aes(ymin = conf.low, ymax = conf.high, y = estimate,
    x = term, colour = .id)) +
  geom_pointrange(position = position_dodge(width = 0.4)) +
  coord_flip() +
  theme_pbs() +
  xlab("") + ylab(en2fr("Coefficient",translate=french, allow_missing=TRUE)) +
  labs(colour = en2fr("Survey",translate=french, allow_missing=TRUE)) +
