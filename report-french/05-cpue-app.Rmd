...
\begin{align}
  ...
  \mu_i &= \exp \left(
  \bm{X}_i \bm{\beta}
  + \alpha^\mathrm{lieu}_{j[i]}
  + \alpha^\mathrm{lieu-annee}_{k[i]}
  + \alpha^\mathrm{navire}_{l[i]}
  \right),\\
\alpha^\mathrm{locality}_j &\sim
  \mathrm{Normal}(0, \sigma^2_{\alpha \; \mathrm{lieu}}),\\
 (\#eq:cpue-locality-year)
\alpha^\mathrm{locality-year}_k &\sim
  \mathrm{Normal}(0, \sigma^2_{\alpha \; \mathrm{lieu-annee}}),\\
 (\#eq:cpue-vessel)
\alpha^\mathrm{vessel}_l &\sim
  \mathrm{Normal}(0, \sigma^2_{\alpha \; \mathrm{navire}}),
\end{align}
...
gfplot:::plot_dfo_localities(unique(dfleet_modern[[1]]$locality_code), french=french)
```
...
gfplot:::plot_dfo_localities(unique(dfleet_modern[[2]]$locality_code), french=french)
```
...
g2 <- gg_cpue_hist$depth + ylab("Nombre de sorties") + xlab(en2fr("Depth", translate=french,allow_missing=TRUE, case="sentence")) +
  labs(fill = "Le voyage\na attrapé la morue\ndu Pacifique") +
  theme(legend.position = c(0.95, 0.95), legend.background = element_rect(fill = "white"),
    legend.justification = c(1, 1)) +
...
dfleet_hist %>%
  ...
  mutate(variable = replace(variable,variable == "Month", "Mois"))%>%
  mutate(variable = replace(variable,variable == "Locality", "Lieu"))%>%
  mutate(variable = replace(variable,variable == "Depth", "Profondeur"))%>%
  filter(area == "3CD") %>%
...
dfleet_modern %>% make_bubble_dat() %>%
  mutate(variable = replace(variable,variable == "Month", "Mois"))%>%
  mutate(variable = replace(variable,variable == "Locality code", "Lieu"))%>%
  mutate(variable = replace(variable,variable == "Depth", "Profondeur"))%>%
  mutate(variable = replace(variable,variable == "Vessel", "Navire"))%>%
  filter(area == "3CD") %>%
...
pars[[1]] <- get_cpue_pars(m_historic[[12]], "Historique 3CD") %>% 
  mutate(Parameter = replace(Parameter,Parameter == "locality SD", "ET lieu"))%>%
  mutate(Parameter = replace(Parameter,Parameter == "year-locality SD", "ET année-lieu"))
...
pars[[3]] <- get_cpue_pars(m_modern[[16]], "Moderne 3CD")%>% 
  mutate(Parameter = replace(Parameter,Parameter == "locality SD", "ET lieu"))%>%
  mutate(Parameter = replace(Parameter,Parameter == "vessel SD", "ET navire"))%>%
  mutate(Parameter = replace(Parameter,Parameter == "year-locality SD", "ET année-lieu"))
...
```{r cpue-index-ts-hist, fig.asp=1.15, fig.cap="Modèles de normalisation de la CPUE pour le chalut commercial. Sur tous les graphiques, la ligne noire et la région ombrée en noir indiquent un indice de la CPUE avec seulement un prédicteur d’année. La ligne colorée et les rubans ombrés en couleur indiquent des indices normalisés par un ou plusieurs prédicteurs. Les trois premières rangées illustrent des modèles de normalisation qui comprennent un seul prédicteur, précisé à droite. L’avant-dernière rangée illustre un modèle de normalisation qui inclut tous les prédicteurs dans un modèle. La dernière rangée illustre un modèle de normalisation qui comprend tous les prédicteurs plus les effets aléatoires lieu-par-année (espace-temps). Les interactions lieu et lieu-année sont ajustées comme des effets aléatoires et toutes les autres variables comme des effets fixes."}
...
p <- list()
for (i in seq_along(params$area_name)) {
  p[[i]] <- make_year_locality_plots(m_modern[[toplot[i]]]) + ggtitle(params$area_name[i]) + ylab("Valeur d'interaction d'interception aléatoire dans l'espace journal") + xlab("Année")
}
cowplot::plot_grid(p[[2]], p[[1]], ncol = 1)
```
...
pp <- left_join(pp, orig_dat, by = "year") %>%
  ...
  ungroup() %>%
  mutate(model = gsub("No space-time interactions", "Sans interactions espace-temps", model)) %>%
  mutate(model = gsub("Space-time interactions", "Interactions espace-temps", model)) %>% 
  mutate(variable = gsub("^y$", "Données sans interactions espace-temps", variable)) %>%
  mutate(variable = gsub("^y_w_int$", "Données avec interactions espace-temps", variable)) %>% 
  mutate(variable = forcats::fct_rev(variable))
  
  #Don't reverse factor levels for "model" with French, reverse "variable" instead. 
  #Need to keep factors in same order in both languages so colours and plots match caption
