...
# NORMALISATION DES CPUE COMMERCIALES 
...
bind_rows(pars) %>%
  knitr::kable(caption = "Écart-type (ET) de l’effet aléatoire et estimations des paramètres de puissance ($p$) et de dispersion ($\\phi$) du modèle d’observation de Tweedie.", booktabs = TRUE, linesep = "", escape = FALSE, format = "pandoc")
```
...
```{r cpue-coef-plot3, fig.asp=1.35, echo=FALSE, fig.cap="Effets aléatoires du lieu-par-année (espace-temps) pour le modèle de normalisation des CPUE commerciales historiques="Coefficients à effet fixe pour le modèle de normalisation des CPUE commerciales modernes.", fig.width=10.5}
p <- list()
for (i in seq_along(params$area_name)) {
...
```{r cpue-coef-plot1-modern, fig.asp=1.2, echo=FALSE, fig.cap=" Les points et les segments épais et minces représentent les moyennes et les intervalles de confiance de Wald à 50\\% et 95\\%."}
toplot <- c(8, 16)
p <- list()
for (i in seq_along(params$area_name)) {
