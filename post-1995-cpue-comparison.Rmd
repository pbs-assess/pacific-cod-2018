...
library("dplyr")
library("ggplot2")
devtools::install_github("pbs-assess/gfplot", ref = "eae5d3d5", 
  dependencies = FALSE)
library("gfplot")
...
Switch to a fishing year that starts April 1. (This is now in the newest version of the package too.)
...
```{r}
# this had an issue in gfplot as of this hash; rightmost.closed = TRUE
bin_clean <- function(x, bins, clean = TRUE) {
...
dfleet <- dfleet %>%
  mutate(depth_bin = bin_clean(best_depth, depth_bands))
...
m1996 <- fit_cpue_index(dfleet,
  ...
  f(locality) + depth_bin + f(latitude),
  formula_lognormal = log(spp_catch/hours_fished) ~ year_factor +
  f(vessel) + f(locality) + depth_bin + f(latitude))
```
